...
function App() {
  ...
  const getAllData = () => {
    axios.get('http://localhost:1338/api/products').then((result) => {
  ...
  const getAllOrders = () => {
    axios.get('http://localhost:1338/api/orders').then((result) => {
      setGetResult(JSON.stringify(result.data.data, null, 2));
  ...
  const getDataById = () => {
    axios
      .get('http://localhost:1338/api/products/' + getId)
      .then((result) =>
        setGetResult(JSON.stringify(result.data.data, null, 2))
      );
  };
  ...
  const putData = () => {
    axios
      .put(
        'http://localhost:1338/api/products/' + putId,
        {
          data: {
            name: putName,
            price: putPrice,
  ...
  const deleteDataById = () => {
    axios
      .delete('http://localhost:1338/api/products/' + deleteId, {
        headers: {
          Authorization: 'Bearer ' + token,
        },
      })
